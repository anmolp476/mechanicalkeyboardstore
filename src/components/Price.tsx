...
const Price = ({ price, id, options }: Props) => {
  ...
  useEffect(() => {
    ...
    setTotal(quantity * (options ? price + options[selectedButt].additionalPrice : price))
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex justify-between w-full p-2 ring-1 ring-black">
          ...
          <div className="flex gap-4 items-center">
            <button onClick={() => setQuantity((curr) => (curr > 1 ? curr-1 : 1))}>{"<"}</button>
            <span>{quantity}</span>
            <button onClick={() => setQuantity((curr) => (curr > 99 ? 99 : curr + 1))}>{">"}</button>
          </div>
        </div>
